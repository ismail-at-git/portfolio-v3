                    ...
                    <div className="space-y-12 border-l-2 border-border pl-6 relative">
                        {internships.map((job, i) => (
                            <div key={i} className="relative">
                                <span className="absolute -left-[33px] top-1 px-1 bg-background text-blue-500">
                                    <div className="w-4 h-4 rounded-full border-2 border-blue-500 bg-background" />
                                </span>
                                <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between mb-2">
                                    <h3 className="text-xl font-bold text-foreground">{job.role}</h3>
                                    <span className="text-sm font-medium text-blue-500">{job.period}</span>
                                </div>
                        ...
                        <div className="space-y-6">
                            {certifications.map((cert, i) => (
                                <div key={i} className="pb-6 border-b border-border/50 last:border-0 last:pb-0">
                                    ...
                                    <span className="text-xs font-medium px-2 py-1 bg-blue-500/10 text-blue-500 rounded ring-1 ring-blue-500/20">
                                        {cert.date}
                                    </span>
                                </div>
                            ))}
